...
class Cache{
	...
	{
		$this->config['DATA_CACHE_PATH'] = isset($config['DATA_CACHE_PATH'])?$config['DATA_CACHE_PATH']:'./data/db_cache/';
		$this->config['DATA_CACHE_TIME'] = isset($config['DATA_CACHE_TIME'])?$config['DATA_CACHE_TIME']:0;
		$this->config['DATA_CACHE_CHECK']=isset($config['DATA_CACHE_CHECK'])?$config['DATA_CACHE_CHECK']:false;
		$this->config['DATA_CACHE_FILE']=isset($config['DATA_CACHE_FILE'])?$config['DATA_CACHE_FILE']:'cachedata';
		$this->config['DATA_CACHE_SIZE']=isset($config['DATA_CACHE_SIZE'])?$config['DATA_CACHE_SIZE']:'15M';
		$this->config['DATA_CACHE_FLOCK']=isset($config['DATA_CACHE_FLOCK'])?$config['DATA_CACHE_FLOCK']:true;
		$this->_checkDir();
		$this->workat($this->config['DATA_CACHE_PATH'].$this->config['DATA_CACHE_FILE']);
	}
	private function _checkDir()
	{
		...
		if(!is_writable($this->config['DATA_CACHE_PATH']))
		{
	...
	{
		$key=md5($name);
		$expire=is_null($expire)?$this->config['DATA_CACHE_TIME']:$expire;
		if(empty($name))
		{
			return false;
		}
		...
		$expire=time()+$expire;
		$check=$this->config['DATA_CACHE_CHECK']?md5($data):'';
	...
	protected function lock($is_block,$whatever=false){
		if($this->config['DATA_CACHE_FLOCK'])
	...
	private function _format($truncate=false){
		if($this->lock(true,true)){
			...
			$this->max_size = $this->_parse_str_size($this->config['DATA_CACHE_SIZE'],15728640);
