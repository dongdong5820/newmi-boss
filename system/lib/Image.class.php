...
{//类定义开始
    ...
    static function buildImageVerify($width=48,$height=22,$randval=NULL,$verifyName='verify')
    {
        ...
        shuffle($arrStr);
        // 生成字母+数字+字母+数字的验证码
        $text[] = array_pop( $arrStr );
        $text[] = array_pop( $arrStr );
        $text[] = array_pop( $arrStr );
        $text[] = array_pop( $arrStr );

        $_SESSION[$verifyName]= implode($text);
        // 保持宽度和搞定的比例		
        //$width = ($height*3+7) > $width ? ($height*3+7) : $width;

        $im = imagecreate($width,$height);
        $bgcolor = imagecolorallocatealpha( $im, 255, 255, 255, 0 );
        $fgcolor = imagecolorallocate( $im, 0, 0, 0 );
        //$bgcolortrans = ImageColorTransparent($im, $bgcolor); /* 设置背景色为透明 */
        putenv('GDFONTPATH=' . realpath('.'));
        //$fontfile[] = dirname(__FILE__) . '/msyh.ttf';
		$fontfile[] = dirname(__FILE__) . '/BOOKOSI.TTF';
		shuffle($fontfile);
        for($i=0;$i<4;$i++) {
            imagettftext($im, mt_rand(14,20), mt_rand(-20,20), 18+16*$i, 23, $fgcolor, $fontfile[0], $text[$i]);
		}
...
//		$borderColor = imagecolorallocate($im, 100, 100, 100);                    //边框色
        ...
        //$stringColor = imagecolorallocate($im,mt_rand(0,200),mt_rand(0,120),mt_rand(0,120));  // 字体的颜色
