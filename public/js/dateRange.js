    ...
    var defaults = {
		...
		aRecent15Days : 'aRecent15Days',//最近15天
	...
	this.periodObj[__method.mOpts.aRecent15Days] = 14;
...
pickerDateRange.prototype.checkDateRange = function(startYmd, endYmd) {
    ...
    if(this.mOpts.dayRangeMax && Math.abs(eTime - sTime) > 86400000 * this.mOpts.dayRangeMax){
        alert('所选日期跨度最大不能超过'+this.mOpts.dayRangeMax+'天');        
        return false;
    }
    return true;
}
...
pickerDateRange.prototype.show = function(isCompare, __method) {
    ...
    $("#" + this.calendarId).css('left', left-12  + 'px');
    //$("#" + this.calendarId).css('top', pos.top + (offsetHeight ? offsetHeight- 1 : (__method.mOpts.theme=='ta'?35:22)) + 'px');
	$("#" + this.calendarId).css('top', pos.top + (__method.mOpts.theme=='ta'?35:22) + 2 + 'px');
	//第一次显示的时候，一定要初始化输入框
...
pickerDateRange.prototype.close = function(btnSubmit) {
	...
	if(btnSubmit){
		...
		if (this.mOpts.shortOpr === true){
			...
			/*if($('.ta_calendar td.today').length){
